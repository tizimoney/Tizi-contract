...
contract MainVault is ISharedStructs, ReentrancyGuard {
    ...
    address public usdc;
    ITransmitter public transmitter;
    ...
    function bridgeOut(
        ...
        (uint256 liquidity, ) = IDepositHelper(helperAddr).calculateLiquidity();
        require(_amount <= liquidity, "Amount is larger than liquidity");
    ...
    function setVault(address[] memory _vault) external onlyAdmin {
        (, bool isAllow) = IDepositHelper(helperAddr).calculateLiquidity();
    ...
    function setCCIPRouter(address _ccipRouter) external onlyAdmin {
        require(_ccipRouter != address(0) && _ccipRouter != address(ccipRouter), "Wrong address");
        ccipRouter = IRouterClient(_ccipRouter);
    }
