...
contract SubVault {
    ...
    function withdrawableAmount(uint256 chainId, address farm) external view returns (uint256) {
        ...
        uint256 maxAmount = IFarm(farm).withdrawableAmount();
        return maxAmount;
    }
    ...
    function getCCIPFee(
        ...
        fee = ccipRouter.getFee(destinationChainSelector, evm2AnyMessage);
        return fee;
    }
    ...
    /// @param receiver The address of mainVault.
    ...
    function bridgeOut(
        ...
        require(receiver == mainVault, "Receiver is not vault");
        IERC20(usdcAddr).approve(address(tokenMessenger), amount);
    ...
    function setMainVault(address newVault) external onlyAdmin {
        require(newVault != address(0) && newVault != mainVault, "Wrong vault address");
        mainVault = newVault;
    }
    ...
    function exitFarm(uint256 chainId, address farm, uint256 amount) public onlyManager {
        ...
        if (amount == 0) {
            amount = IERC20(usdcAddr).balanceOf(farm);
            require(amount > 0, "Amount must be greater than zero");
        }
    ...
    function enterFarm(uint256 chainId, address farm, uint256 amount) public onlyManager returns (bool) {
        ...
        require(amount > 0, "Amount must be greater than zero");
        require(IERC20(usdcAddr).transfer(farm, amount) == true, "transfer failure");
    ...
    function deposit(uint256 chainId, address farm, uint256 amount) public onlyManager {
        ...
        require(
            IERC20(usdcAddr).balanceOf(farm) >= amount,
