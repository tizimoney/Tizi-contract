...
import {IOFTSubscriber} from "../layerzero/token/oft/interfaces/IOFTSubscriber.sol";
...
abstract contract CrossChainToken {
    ...
    function _tryNotifyReceiver(
        ...
        bytes memory data = abi.encodeCall(
            IOFTSubscriber.notifyCredit,
            (srcChainId, initiator, sender, address(this), amount)
        );
        assembly {
            success := call(
                gas(), // gas remaining
