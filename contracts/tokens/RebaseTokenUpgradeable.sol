...
abstract contract RebaseTokenUpgradeable is ERC20Upgradeable {
    ...
    function ERC20Num() public view returns (uint256 supply) {
        supply = ERC20Upgradeable.totalSupply();
    }

    function sharesNum() public view returns (uint256 shares) {
        RebaseTokenStorage storage $ = _getRebaseTokenStorage();
        shares = $.totalShares;
    }
    ...
    function _setRebaseIndex(uint256 index) internal virtual {
        ...
        if (currentIndex != index) {
            ...
            uint256 totalSupplyBefore = $.totalShares.toTokens(currentIndex) +
                constantSupply;
            uint256 totalSupplyAfter = $.totalShares.toTokens(index) +
                constantSupply;
            emit RebaseIndexUpdated(
                msg.sender,
                index,
                totalSupplyBefore,
                totalSupplyAfter
