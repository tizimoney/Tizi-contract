 ...
 */
abstract contract CrossChainRebaseTokenUpgradeable is
    RebaseTokenUpgradeable,
    CrossChainToken
{
    constructor(uint256 chainId) CrossChainToken(chainId) {}
    ...
    function _getCrossChainRebaseTokenStorage()
        private
        pure
        returns (CrossChainRebaseTokenStorage storage $)
    {
     ...
     */
    function __CrossChainRebaseToken_init(
        string memory name,
    ...
    function __CrossChainRebaseToken_init_unchained()
        internal
        onlyInitializing
    {}
    ...
    function _rebaseNonce() internal view returns (uint256 nonce) {
        CrossChainRebaseTokenStorage
            storage $ = _getCrossChainRebaseTokenStorage();
        nonce = $.nonce;
    }
