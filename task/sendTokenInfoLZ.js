 ...
 *
 * Usage:
 * npx hardhat sendTokenInfoLZ --chaineid <chainEID>
 * --network <network>
 *
 * Permissions:
 * Admin
...
module.exports = async function (taskArgs, hre) {
    //获得合约实例
    const contract = await ethers.getContract( "SubTokenLayerZero" )
    console.log(contract.target)

    const DESTINATION_CHAIN = taskArgs.chaineid
    // set token code
    const setTx = await contract.setTokenCode()
    console.log(`set token code ${setTx.hash}`)
    await setTx.wait()

    // get token code
    const code = await contract.getTokenCode()
